...
export default function RiskMap({ markers = [], center = [25.3176, 82.9739], zoom = 9 }) {
            ...
            <Popup>
              <div className="p-1 max-w-xs font-sans">
                <div className="flex items-center justify-between gap-2 border-b pb-1 mb-1.5">
                  <span className="font-bold text-xs text-slate-900">{m.project_id}</span>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                    m.risk_level === 'CRITICAL' ? 'bg-red-100 text-red-800' :
                    m.risk_level === 'HIGH' ? 'bg-orange-100 text-orange-800' :
                    m.risk_level === 'MEDIUM' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'
                  }`}>
                    {m.risk_level} ({m.risk_score})
                  </span>
                </div>
                <h4 className="font-semibold text-xs text-slate-800 leading-tight mb-1">{m.project_name}</h4>
                <p className="text-[10px] text-slate-500 mb-2">{m.district}, {m.state} • ₹{m.sanction_amount}L</p>
                <div className="grid grid-cols-2 gap-1 text-[10px] bg-slate-50 p-1.5 rounded mb-2 border border-slate-200">
                  <div>Phys Prog: <b>{m.physical_progress}%</b></div>
                  <div>Fin Prog: <b>{m.financial_progress}%</b></div>
                </div>
                <button
