...
export default function ProjectIntelligence() {
  ...
  const { project, risk_summary, ai_insight, risk_factors, trajectory, predicted_delay_days, evidences } = data;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
          ...
          <span>/</span>
          <span className="text-slate-900 font-semibold">{project.project_id}</span>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate(`/peer-twins?project=${project.project_id}`)}
            className="bg-slate-100 text-slate-700 hover:bg-slate-200 text-xs font-semibold px-3 py-1.5 rounded border border-slate-300 transition"
          >
            View Peer Twins →
          </button>
          <button
            onClick={() => navigate(`/investigation-centre`)}
            className="bg-slate-900 text-white hover:bg-slate-800 text-xs font-semibold px-3 py-1.5 rounded transition"
          >
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">{project.project_name}</h1>
          <p className="text-xs text-slate-500 font-medium flex items-center gap-3">
            <span><MapPin className="w-3.5 h-3.5 inline mr-1" />{project.village}, {project.district}, {project.state}</span>
            <span>•</span>
            <span><Building2 className="w-3.5 h-3.5 inline mr-1" />{project.agency_name}</span>
          </p>
        </div>
        ...
        <div className="bg-red-50 border border-red-200 p-4 rounded-lg flex items-center gap-4 min-w-[220px]">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-red-700 block">System Risk Index</span>
            <div className="flex items-baseline gap-1 mt-0.5">
              <span className="text-3xl font-black text-red-900">{risk_summary.total_score}</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 text-xs">
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Sanction Amount</span>
          <p className="text-base font-extrabold text-slate-900 mt-1">₹{project.sanction_amount} Lakh</p>
        </div>

        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Expenditure</span>
          <p className="text-base font-extrabold text-slate-900 mt-1">₹{project.expenditure} Lakh</p>
        </div>

        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Physical Progress</span>
          <p className="text-base font-extrabold text-slate-900 mt-1">{project.physical_progress}%</p>
        </div>

        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Financial Progress</span>
          <p className="text-base font-extrabold text-slate-900 mt-1">{project.financial_progress}%</p>
        </div>

        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Expected Progress</span>
          <p className="text-base font-extrabold text-slate-700 mt-1">74%</p>
        </div>

        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Predicted Delay</span>
          <p className="text-base font-extrabold text-red-600 mt-1">{predicted_delay_days} Days</p>
        </div>
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm col-span-2 lg:col-span-1">
          ...
          <p className="text-xs font-bold text-slate-800 mt-1">{project.sanction_date}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-500">Click any contributing factor to view detailed proof and baseline evidence</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {risk_factors.map((rf) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">{rf.factor_type}</span>
              ...
              <p className="text-[11px] text-slate-600 line-clamp-2">{rf.description}</p>
              <div className="text-[10px] font-semibold text-slate-500 flex items-center gap-1 pt-1">
                View detailed explanation <ChevronRight className="w-3 h-3 text-slate-400" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-500">Physical progress timeline vs benchmark completion baseline</p>
          </div>
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trajectory} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="expected_progress" stroke="#94a3b8" strokeWidth={2} strokeDasharray="5 5" name="Expected Progress %" />
              <Line type="monotone" dataKey="actual_progress" stroke="#ef4444" strokeWidth={3} name="Actual Progress %" />
              <Line type="monotone" dataKey="expenditure" stroke="#3b82f6" strokeWidth={2} name="Financial Disbursement %" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-500">Chronological ground site photographic inspection log with verified metadata</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {evidences.map((e) => (
            <div key={e.id} className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50 flex flex-col justify-between">
              <div className="h-44 bg-slate-200 relative overflow-hidden">
                <img src={e.file_url} alt={e.evidence_type} className="w-full h-full object-cover" />
                <span className="absolute top-2 left-2 bg-slate-900/90 text-white text-[10px] font-bold px-2 py-0.5 rounded">
                  {e.capture_date}
                </span>
              ...
              <div className="p-3 space-y-1.5 text-xs">
                ...
                <p className="text-[11px] text-slate-600 leading-snug">{e.description}</p>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-4 relative">
            ...
            <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
              ...
              <h3 className="font-extrabold text-slate-900 text-sm uppercase">{selectedFactor.factor_type}</h3>
            </div>

            <div className="space-y-3 text-xs">
              <div>
                <h4 className="font-bold text-slate-900 text-xs">{selectedFactor.title}</h4>
                <p className="text-slate-600 mt-1 leading-relaxed">{selectedFactor.description}</p>
              </div>

              <div className="bg-slate-50 p-3 rounded border border-slate-200 space-y-1">
                <span className="font-bold text-slate-700 text-[11px] block uppercase">Mathematical Evidence Proof:</span>
                <p className="font-mono text-slate-800 text-[11px]">{selectedFactor.evidence}</p>
              </div>

              <div className="bg-amber-50 text-amber-900 p-3 rounded border border-amber-200 text-[11px] font-medium leading-normal">
                <b>Governance Classification:</b> Flagged as High Irregularity Risk. Requires field measurement book verification before final fund disbursement approval.
              </div>
            </div>
