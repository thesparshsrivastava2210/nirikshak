...
export default function InvestigationCentre() {
  ...
  // Modals state
  const [briefModalData, setBriefModalData] = useState<any>(null);
  ...
  const [recommendedAction, setRecommendedAction] = useState(
    "Field Verification & Measurement Book Audit"
  );
  const [assignedTo, setAssignedTo] = useState(
    "Shri Rajesh Kumar (District Nodal Officer)"
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setProjects(pData);
      if (pData.length > 0) setSelectedProjectId(String(pData[0].id));
    } catch (err) {
  ...
  const handleViewBrief = async (invId: number) => {
    try {
  ...
  const handleStatusChange = async (invId: number, newStatus: string) => {
    try {
  ...
  const assignedCases = investigations.filter(
    (i) => i.status === "Assigned"
  ).length;
  const reviewCases = investigations.filter(
    (i) => i.status === "In Review"
  ).length;
  const verifiedCases = investigations.filter(
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto w-full overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-200 pb-4 gap-4">
        ...
        <button
          ...
          className="bg-slate-900 text-white text-xs font-semibold px-4 py-2 rounded-md hover:bg-slate-800 transition flex items-center gap-1.5 shadow-sm cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
        ...
        <div className="bg-white p-3.5 rounded-lg border border-slate-200 shadow-sm">
          ...
          <h3 className="text-xl font-extrabold text-red-900 mt-1">
            {openCases}
          </h3>
        </div>
                  ...
                  <tr key={inv.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="py-3 px-3 text-right space-x-2">
                      <button
                        ...
                        className="bg-slate-900 text-white text-[11px] font-semibold px-2.5 py-1 rounded hover:bg-slate-800 cursor-pointer"
                      >
                        View Brief
                      </button>
                    </td>
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-2xl max-w-2xl w-full p-6 space-y-4 my-8 relative">
            ...
            <div className="border-b border-slate-200 pb-3">
              ...
              <h3 className="font-extrabold text-slate-900 text-base mt-1">
                {briefModalData.brief_title}
              </h3>
              <p className="text-xs text-slate-500">
                {briefModalData.project_details.district},{" "}
                {briefModalData.project_details.state} • Sanction:{" "}
                {briefModalData.project_details.sanction_amount}
              </p>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div className="bg-slate-900 text-slate-100 p-4 rounded-lg space-y-1">
                ...
                <p className="text-xs leading-relaxed">
                  {briefModalData.executive_summary}
                </p>
              </div>

              <div>
                <h4 className="font-bold text-slate-900 uppercase text-[11px]">
                  Primary Risk Signals Flagged:
                </h4>
                <div className="space-y-1 mt-1">
                  {briefModalData.primary_signals.map((sig: string, i: number) => (
                    <p
                      key={i}
                      className="bg-red-50 text-red-900 p-2 rounded border border-red-200 text-[11px]"
                    >
                      • {sig}
                    </p>
                  ))}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <div className="bg-slate-50 p-2.5 rounded border border-slate-200">
                    <b className="block text-[10px] text-slate-500 uppercase">
                      Financial Audit
                    </b>
              ...
              <div>
                ...
                <div className="bg-slate-50 p-3 rounded border border-slate-200 space-y-1 mt-1">
                  {briefModalData.recommended_checklist.map(
                    (chk: string, i: number) => (
                      <p
                        key={i}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form
            ...
            className="bg-white rounded-lg border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-4 relative"
          >
            <button
              type="button"
            ...
            <div className="border-b border-slate-200 pb-2">
              <h3 className="font-extrabold text-slate-900 text-sm uppercase">
                Create New Inspection Case
              </h3>
              <p className="text-xs text-slate-500">
                Assign project to district nodal authority for field verification
              </p>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded px-2.5 py-1.5 font-medium text-slate-900 cursor-pointer"
                >
                  {projects.map((p) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded px-2.5 py-1.5 font-semibold text-slate-900 cursor-pointer"
                  >
            ...
            <div className="pt-3 border-t border-slate-200 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setShowCreateModal(false)}
                className="bg-slate-100 text-slate-700 font-semibold text-xs px-4 py-2 rounded hover:bg-slate-200 cursor-pointer"
              >
                Cancel
              </button>
              <button
