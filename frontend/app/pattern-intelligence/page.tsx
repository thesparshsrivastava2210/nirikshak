...
import { fetchAgencyPatterns, fetchRelationshipGraph } from "@/lib/api";
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto w-full overflow-x-hidden">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {(Array.isArray(agencies) ? agencies : []).map((a, idx) => {
            const highRiskCount =
              typeof a.high_risk_projects === "number"
                ? a.high_risk_projects
                : Array.isArray(a.high_risk_projects)
                  ? a.high_risk_projects.length
                  : 0;
            const findingsList = Array.isArray(a.findings) ? a.findings : [];

            return (
              <div
                key={a.agency_id || a.id || idx}
                className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm space-y-3"
              >
                <div className="flex items-start justify-between border-b border-slate-200 pb-3">
                  <div>
                    <span className="text-[10px] font-bold text-slate-500 uppercase">
                      {a.agency_type || "Implementing Agency"}
                    </span>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-medium bg-slate-50 p-2.5 rounded border border-slate-200">
                  <div>
                    Total Works: <b>{a.total_projects || 0}</b>
                  </div>
                  <div className="text-red-700 font-semibold">
                    High Risk: <b>{highRiskCount}</b>
                  </div>
                  <div>
                    Cost Deviation: <b>{a.cost_deviation_cases || 0}</b>
                  </div>
                  <div>
                    Delays: <b>{a.delay_cases || 0}</b>
                  </div>
                </div>
                ...
                <div className="space-y-1 text-xs">
                  ...
                  {findingsList.map((f: string, i: number) => (
                    <p
                      key={i}
                      className="text-slate-700 text-[11px] font-medium leading-tight flex items-start gap-1"
                    >
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Visualizing network linkages between agencies, active projects, and duplicate overlaps
            </p>
          </div>
          <div className="flex items-center gap-3 text-[11px] font-semibold">
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-slate-800" /> Agency Node
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-full bg-red-500" /> Critical Project
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-full bg-orange-500" /> High Risk Project
            </span>
          </div>
        </div>
        ...
        <div className="relative w-full h-[450px] bg-slate-50 border border-slate-200 rounded-lg overflow-hidden flex items-center justify-center">
          ...
          {selectedNode && (
            <div className="absolute top-4 right-4 bg-slate-900 text-white rounded-lg p-4 shadow-xl border border-slate-700 w-72 text-xs space-y-2 z-10">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                <span className="font-bold text-slate-300 uppercase">
                  {selectedNode.group} Details
                </span>
                <button
              ...
              {selectedNode.details && (
                <div className="space-y-1 text-[11px] text-slate-300">
                  {selectedNode.details.cost && (
                    <p>Cost: ₹{selectedNode.details.cost} Lakh</p>
                  )}
                  {selectedNode.details.progress && (
                    <p>
                      Physical Progress: {selectedNode.details.progress}%
                    </p>
                  )}
                  {selectedNode.details.risk && (
                    <p className="font-bold text-red-400">
                      Risk Score: {selectedNode.details.risk}/100
                    </p>
                  )}
                </div>
              )}
              {selectedNode.group === "Project" && (
                <button
                  onClick={() =>
                    router.push(
                      `/projects/${selectedNode.label.split(" ")[0]}`
                    )
                  }
                  className="w-full bg-slate-800 hover:bg-slate-700 text-white text-[11px] font-semibold py-1 rounded mt-2 text-center cursor-pointer"
                >
