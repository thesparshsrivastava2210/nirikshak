...
import { fetchPotentialOverlaps } from "@/lib/api";
import { MapPin, Sliders, ShieldAlert } from "lucide-react";
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          <div className="py-8 text-center text-slate-500 text-xs animate-pulse">
            Scanning spatial proximity & descriptions...
          </div>
        ) : (
          <div className="space-y-4">
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between bg-amber-50 border border-amber-200 p-3 rounded-md text-xs">
                  ...
                  <div className="flex items-center gap-4 text-[11px] font-semibold text-amber-900">
                    <span>
                      Distance: <b>{item.distance_meters} metres</b>
                    </span>
                    <span>
                      Similarity Score: <b>{item.similarity_score}%</b>
                    </span>
                  </div>
                </div>
                ...
                {(() => {
                  const pA = item.project_a || {
                    project_id: "P1045",
                    project_name:
                      "Construction of Community Hall & Skill Center",
                    description: "Multipurpose community hall construction",
                    district: "Varanasi",
                    estimated_cost: 68.0,
                  ...
                  const pB = item.project_b || {
                    ...
                    project_name:
                      item.project_name ||
                      "Construction of Public Community Centre Building",
                    description:
                      item.status_note ||
                      "Nearby public community centre building",
                  ...
                  return (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                      ...
                      <div className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-2">
                        ...
                        <button
                          onClick={() =>
                            router.push(`/projects/${pA.project_id}`)
                          }
                          className="w-full bg-slate-900 text-white text-[11px] font-semibold py-1.5 rounded hover:bg-slate-800 text-center block mt-2 cursor-pointer"
                        >
                      ...
                      <div className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-2">
                        ...
                        <p className="text-[11px] text-slate-600 leading-snug">
                          {pB.description || pB.project_name}
                        </p>
                        <div className="pt-2 border-t border-slate-200 flex justify-between text-[11px] font-medium text-slate-500">
                        ...
                        <button
                          onClick={() =>
                            router.push(`/projects/${pB.project_id}`)
                          }
                          className="w-full bg-slate-900 text-white text-[11px] font-semibold py-1.5 rounded hover:bg-slate-800 text-center block mt-2 cursor-pointer"
                        >
