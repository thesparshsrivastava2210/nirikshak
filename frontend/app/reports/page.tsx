  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      html2canvas(reportRef.current, { scale: 2 }).then((canvas) => {
        ...
        pdf.save(`NIRIKSHAK_Risk_Report_${districtFilter}_2024.pdf`);
      });
    } catch (e) {
  ...
  if (loading || !report) {
    return (
      <div className="p-8 text-center text-slate-500 font-medium animate-pulse">
        Generating Governance Risk Report...
      </div>
    );
  }

  const { metrics, critical_projects } = report;
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto w-full overflow-x-hidden">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        ...
        <button
          ...
          className="bg-slate-900 text-white text-xs font-semibold px-4 py-2 rounded-md hover:bg-slate-800 transition flex items-center gap-2 shadow-sm cursor-pointer"
        >
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex flex-wrap items-center gap-4 text-xs">
        ...
        <div>
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-300 rounded px-2.5 py-1 text-slate-800 font-bold focus:outline-none cursor-pointer"
          >
      ...
      <div
        ref={reportRef}
        className="bg-white rounded-lg border border-slate-300 p-8 shadow-sm space-y-6"
      >
        {/* Report Official Banner */}
        <div className="border-b-2 border-slate-900 pb-4 flex justify-between items-start">
          <div>
            <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest block">
              GOVERNMENT OF INDIA • MPLADS MONITORING PORTAL
            </span>
            <h2 className="text-xl font-black text-slate-900 tracking-tight mt-1">
              {report.report_title}
            </h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              {report.authority_scope} • Date: {report.generated_date}
            </p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-3 bg-slate-100 rounded border border-slate-200">
            ...
            <p className="text-lg font-black text-slate-900 mt-0.5">
              ₹{metrics.total_sanctioned_cr} Cr
            </p>
          </div>
          <div className="p-3 bg-slate-100 rounded border border-slate-200">
            <span className="text-[10px] font-bold text-slate-500 uppercase">
          ...
          <div className="p-3 bg-red-50 rounded border border-red-200">
            ...
            <p className="text-lg font-black text-red-900 mt-0.5">
              {metrics.critical_risk_count} Works
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
            Critical Risk Projects Requiring Field Inspection (
            {critical_projects.length})
          </h3>
            ...
            <tbody className="divide-y divide-slate-200 text-xs font-medium">
              {critical_projects.map((p: any) => (
                <tr key={p.project_id}>
                  ...
                  <td className="py-2 px-2 font-extrabold text-red-700">
                    {p.risk_score} / 100
                  </td>
                </tr>
              ))}
        ...
        <div className="pt-6 border-t border-slate-200 flex justify-between items-center text-[10px] text-slate-400 font-mono">
          <span>Generated by NIRIKSHAK Intelligence Core</span>
          <span>
            Authenticity Verified • Ministry of Statistics & Programme
