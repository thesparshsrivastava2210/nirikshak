...
import { fetchPeerTwins, fetchProjects } from "@/lib/api";
import { GitCompare, AlertTriangle } from "lucide-react";

function roundDelta(val: number) {
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto w-full overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-200 pb-4 gap-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <select
            ...
            className="bg-white border border-slate-300 rounded-md px-3 py-1.5 font-semibold text-slate-900 focus:outline-none shadow-sm cursor-pointer"
          >
            {allProjects.map((p) => (
      ...
      <div className="bg-slate-900 text-white rounded-lg p-5 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
            Subject Project Under Analysis
          </span>
          <h2 className="text-lg font-bold text-white mt-0.5">
            {target_project.project_id} — {target_project.project_name}
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            {target_project.district} • Cost: ₹{target_project.estimated_cost}{" "}
            Lakh • Progress: {target_project.physical_progress}%
          </p>
        </div>

        <div className="bg-slate-800 border border-slate-700 px-4 py-3 rounded-md text-right min-w-[200px]">
          <span className="text-[10px] font-bold text-slate-400 uppercase block">
            Peer Benchmark Variance
          </span>
          <span className="text-xl font-extrabold text-amber-400">
            {benchmarks.cost_variance_pct > 0
              ? `+${benchmarks.cost_variance_pct}%`
              : `${benchmarks.cost_variance_pct}%`}
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <h3 className="text-2xl font-extrabold text-slate-900 mt-1">
            ₹{benchmarks.peer_median_cost} Lakh
          </h3>
          <p className="text-[10px] text-slate-400 mt-0.5">
            Calculated across {peer_twins.length} twin works
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-500 uppercase">
            Peer Average Finance
          </span>
          <h3 className="text-2xl font-extrabold text-slate-900 mt-1">
            {benchmarks.peer_avg_finance}%
          </h3>
          <p className="text-[10px] text-slate-400 mt-0.5">
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
          ...
          <h3 className="text-2xl font-extrabold text-slate-900 mt-1">
            ₹{target_project.estimated_cost} Lakh
          </h3>
          <p className="text-[10px] text-amber-700 font-semibold mt-0.5">
            {benchmarks.status_summary}
          </p>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg text-xs text-amber-900 font-semibold flex items-center gap-3">
        ...
        <span>
          <b>Peer Twin Insight:</b> {benchmarks.status_summary} Physical progress (
          {target_project.physical_progress}%) lags peer average (
          {benchmarks.peer_avg_progress}%) by{" "}
          {roundDelta(
            benchmarks.peer_avg_progress - target_project.physical_progress
          )}{" "}
          percentage points.
        </span>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          ...
          <span className="text-xs font-semibold text-slate-500">
            Feature Weights: Type 35% | Cost 25% | Geo 20% | Scale 20%
          </span>
        </div>
            ...
            <tbody className="divide-y divide-slate-200 text-xs">
              ...
              <tr className="bg-slate-900 text-white font-bold">
                ...
                <td className="py-3 px-3 text-slate-300">
                  {target_project.district}
                </td>
                <td className="py-3 px-3 text-white">
                  ₹{target_project.estimated_cost}L
                </td>
                <td className="py-3 px-3 text-amber-300">
                  {target_project.physical_progress}%
                </td>
                <td className="py-3 px-3 text-slate-300">
                  {target_project.financial_progress}%
                </td>
                ...
                <tr key={p.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="py-3 px-3 text-right">
                    <button
                      ...
                      className="text-slate-800 hover:text-slate-950 font-bold hover:underline cursor-pointer"
                    >
                      Compare →
                    </button>
                  </td>
